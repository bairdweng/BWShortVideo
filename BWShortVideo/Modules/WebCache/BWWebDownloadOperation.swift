...
class BWWebDownloadOperation: Operation {
    // 下载回调block
    var progressBlock: BWWebDownloaderProgressBlock?
    var responseBlock: BWWebDownloaderResponseBlock?
    var completedBlock: BWWebDownloaderCompletedBlock?
    ...
    // 初始化数据
    init(request: URLRequest, response: @escaping BWWebDownloaderResponseBlock, progress: @escaping BWWebDownloaderProgressBlock, completed: @escaping BWWebDownloaderCompletedBlock, cancel: @escaping BWWebDownloaderCancelBlock) {
        super.init()
...
extension BWWebDownloadOperation: URLSessionDataDelegate, URLSessionDelegate {
    // 网络资源下载请求获得响应
    ...
    func urlSession(_: URLSession, dataTask _: URLSessionDataTask, didReceive data: Data) {
        ...
        if progressBlock != nil {
            progressBlock?(Int64(imageData?.count ?? 0), expectedSize ?? 0, data)
        }
    }

    // 网络缓存数据复用
    func urlSession(_: URLSession, dataTask _: URLSessionDataTask, willCacheResponse proposedResponse: CachedURLResponse, completionHandler: @escaping (CachedURLResponse?) -> Void) {
